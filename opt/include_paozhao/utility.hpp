...
namespace SimpleWeb {
  ...
  inline std::string getValue(SimpleWeb::CaseInsensitiveMultimap &mp, const std::string &key) {
    SimpleWeb::CaseInsensitiveMultimap::iterator it = mp.find(key);
    if(it != mp.end()) {
      return it->second;
    }
    else
      throw "Could not find key in the map";
  };
